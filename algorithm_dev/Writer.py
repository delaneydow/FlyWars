...
class Writer:
    def __init__(self): 
        ...
        self.frame_file.write("frame,time,vision_latency_ms,pipeline_latency_ms,"
                              "ndet,ntrack,temp,cooldown\n")

        self.track_file.write("frame,track,x,y,vx,vy,speed,state,cov\n")

        self.fire_file.write("frame,track_id,score,aim_x,aim_y,"
                             "hit_time,hit_confirmed,redirects\n")

    def log_frame(self, f):
            self.frame_file.write(
                f"{f['frame']},{f['time']},"
                f"{f['vision_latency_ms']}, {f['pipeline_latency_ms']},"
                f"{f['ndet']},{f['ntrack']},"
                f"{f['temp']},{f['cooldown']}\n"
            )
